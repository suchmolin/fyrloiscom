...
export default function PromocionesCursos({ promoList, cursoId }) {
  ...
  return (
    <div className="w-full bg-gray-100 py-20 flex flex-col md:flex-row gap-20 md:gap-10 lg:gap-20 justify-center items-center overflow-hidden">
      {promos.map((prom) => (
        <>
          <CardPromo key={prom.id} item={prom} curso={curso} />
        </>
      ))}
    </div>
  )
